...
public class Reward {
	...
	private boolean diceRoll(Player player) {
		debug("Pre-event chance: " + this.getChance());
		ChanceCalculateEvent event = new ChanceCalculateEvent(this, player);
		Bukkit.getPluginManager().callEvent(event);
		double eventChance = event.getChance();
		debug("Post-event chance: " + eventChance);
		double random = RANDOM.nextDouble() * 100;
		if (random < eventChance) {
			debug(random + " < " + eventChance + ", executing reward");
			return true;
		} else {
			debug(random + " !< " + eventChance + ", doing nothing");
			return false;
		}
	}
}
